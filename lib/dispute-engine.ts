...
export async function resolveDispute(params: {
  ...
  await supabase.from('disputes').update({
    ...
    employee_penalty: params.employee_penalty ?? null,
    resolved_by: params.resolved_by,
    resolved_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  }).eq('id', params.dispute_id);

  // Apply employee score penalty if specified
  if (params.employee_penalty && params.employee_penalty > 0 && dispute.employee_id) {
    await applyEmployeePenalty(dispute.employee_id, params.employee_penalty);
  }
...
async function applyEmployeePenalty(
  employeeId: string,
  ...
  const newScore = Math.max(1.0, (employee.score as number) - scorePenalty);
  ...
  await Promise.all([
    supabase
      ...
      .eq('id', employeeId),
    supabase.from('employee_score_history').insert({
      employee_id: employeeId,
      score_before: employee.score,
...
export async function getDisputeWithMessages(disputeId: string) {
  ...
  const [disputeResult, messagesResult] = await Promise.all([
    supabase
      .from('disputes')
      .select('*, job:jobs(*, customer:customers(*), employee:employees(*)), customer:customers(*), employee:employees(*)')
      .eq('id', disputeId)
      .single(),
    supabase
