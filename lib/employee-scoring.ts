...
export async function recalculateEmployeeScore(employeeId: string): Promise<number> {
  const supabase = await createServiceClient();

  const { data: jobs } = await supabase
    .from('jobs')
    .select('id, status, employee_started_at, scheduled_date, scheduled_window')
  ...
  await supabase.from('employee_score_history').insert({
    ...
    score_before: currentEmployee?.score ?? null,
