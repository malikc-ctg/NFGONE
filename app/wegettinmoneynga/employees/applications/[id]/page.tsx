...
export default function ApplicationDetailPage() {
  ...
  useEffect(() => {
    async function fetchApp() {
      try {
        const res = await fetch(`/api/wegettinmoneynga/employee-applications/${params.id}`);
  ...
  const handleSaveReview = async () => {
    ...
    try {
      ...
      // If approved, we could potentially show the "Add Employee" logic here or redirect
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => router.push('/wegettinmoneynga/employees')} className="h-8 w-8">
                ...
                {status === 'Approved' && (
                  <div className="pt-4 border-t border-border mt-4">
                    <p className="text-xs text-muted-foreground mb-3 leading-tight">
                      This applicant has been approved. You can now invite them as a employee.
                    </p>
                    <Link href="/wegettinmoneynga/employees">
