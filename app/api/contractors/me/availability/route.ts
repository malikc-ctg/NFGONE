...
export async function GET() {
  try {
    ...
    // Get availability settings (weekly grid stored as JSONB in contractor notes or a dedicated field)
    ...
    try {
      const parsed = JSON.parse(contractor?.notes || '{}');
...
export async function PUT(request: Request) {
  try {
    ...
    try {
      existingNotes = JSON.parse(contractor?.notes || '{}');
    } catch {}
