  ...
  async function handleReset(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: `${window.location.origin}/api/auth/callback?next=/reset-password?portal=admin`,
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-background text-foreground dark px-4">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Reset admin password</p>
        </CardHeader>
        <CardContent>
          {sent ? (
            ...
            <form onSubmit={handleReset} className="space-y-4">
              ...
              <div className="text-center text-sm pt-2">
                <Link href="/sobadmin/login" className="text-primary hover:underline">
                  Back to login
                </Link>
              </div>
            </form>
