...
function NewJobForm() {
  ...
  // Preselect customer from query param and advance to Service step
  useEffect(() => {
    if (customerIdParam && customers.length > 0) {
      ...
      if (match) {
        setForm(f => ({
          ...
          zone_id: match.zone_id ?? f.zone_id,
        }));
        setStep(1);
      }
    }
...
export default function NewJobPage() {
  return (
    <Suspense fallback={<div className="p-6 text-muted-foreground">Loading job creator...</div>}>
      <NewJobForm />
    </Suspense>
  );
}
