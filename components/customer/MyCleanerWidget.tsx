...
export function MyCleanerWidget({ employee }: MyCleanerWidgetProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="p-5 flex flex-col items-center justify-center flex-1 text-center">
        ...
        <h4 className="font-bold text-lg text-slate-900 leading-tight">{employee.full_name}</h4>
        ...
        <p className="text-xs text-slate-500 my-3 italic px-2">
          &ldquo;{(() => {
            try {
              return JSON.parse(employee.notes || '{}').bio || 'Dedicated professional cleaner committed to making your home sparkle.';
            } catch {
              return 'Dedicated professional cleaner committed to making your home sparkle.';
            }
          })()}&rdquo;
        </p>
        <a href={`sms:${employee.phone}`} className="w-full bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-2 text-sm">
